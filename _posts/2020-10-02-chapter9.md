...
الكثير من البيانات تم جمعها من قبل أشخاص مختلفين، فريق من علماء البيانات مثلاً. إذا تم حفظ البيانات في ملفات نصيه، سيحتاج الفريق لتحميل وإرسال البيانات في كل مرة يتم التعديل عليها. الملفات النصية وحدها لا توفر طريقة مناسبه لإستعادة ومشاركتها ليتم استخدامها بين عدد مختلف من المحللين. هذه المشكلة، إضافة إلى مشاكل أخرى، تصعب عمل الملفات النصية لفريق التحليل أو اذا كان حجم البيانات كبيراً.
...
*مخطط العلاقات Relation schema* تحتوي على أسماء الأعمدة، أنواع البيانات وقيودها. مثلاً، مخطط جدول `purchases` يحدد أن هناك الأعمدة `name`، `product`، `retailer` و `date purchased`، ويوضح أيضاً أن كل عامود يحتوي على نص.
...
- تناسق البيانات: يمكن لهذه الأنظمة فرض قيود على المحتوى المُدخل، مثلاً، العامود `GPA` يجب أن يحتوي فقط على رقم عشري من 0. 0 حتى 4. 0.
...
لاحظ أن:
- **جميع ما في [داخل الأقواس] هي أوامر اختباريه.** ليكون أمر الاستعلام صحيح يجب أن يحتوي على `SELECT` و `FROM`.
...
الاستعلام `SELECT *` يجلب لنا جميع الأعمدة. لعرض فقط عامود `retailer`، نقوم بإضافتها كالتالي:
...
إذا أردنا عرض القيم في الجدول بدون تكرار، نقوم بإضافة `DISTINCT`:
...
لاحظ أن بإمكاننا استخدام مسميات مستعارة للجداول **Alias** باستخدام `AS` لكي يظهر العامود بمسمى جديد. لكن هذا لا يغير شيء باسم العامود الأصلي
...
حتى الآن، تعاملنا مع بيانات موجودة في الجداول، يعني ذلك، كل النتائج التي ظهرت سابقاً هي أجزاء من المدخلات في الجداول. ولكن لتطبيق تحليل للبيانات، نحتاج للقيام ببعض عمليات التجميع على بياناتنا. في SQL، يطلق عليها **دوال التجميل Aggregate Functions**.
...
- `GROUP BY` تستقبل أسماء عواميد وتقوم بجمعها كما في دالة [pd.DataFrame.groupby](https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.groupby.html) في بانداز.
...
لنقل أن لدينا عميل ذو ذوق عالي ويريد فقط تلك المتاجر التي تطبيع منتجات بسعر أعلى من $700. لاحظ أن يجب علينا استخدام `HAVING` لإيجاد النتيجة من عامود تم تجميعه؛ لا يمكننا استخدام `WHERE` لفلترة نتائج عامود مُجَمع. لإيجاد قائمة بالمتاجر التي تبيع المنتجات المفضلة لعميلنا نقوم بالتالي:
...
- `ORDER BY` تساعدنا على عرض البيانات بشكل مُرتب بِنَاءًا على القيم داخل العامود. بشكل تلقائي، `ORDER BY` تستخدم الترتيب التصاعدي `ASC` ولكن يمكننا عكسها وعرض البيانات بشكل تنازلي باستخدام `DESC`.
...
لنفرض مثلاً أننا متجر لمنتجات القطط ولدينا قاعدة بيانات بالقطط التي في متجرنا. لدينا **جدولين** مختلفين: `names` و `colors`. جدول `names` يحتوي على العامودان:
...
لاحظ وجود قيم مفقوده في كل جدول. القيمة 3 في العامود `cat_id` مفقوده من جدول `names`، والقيمة 4 من عامود `cat_id` مفقوده في جدول `colors`
...
لإيجاد لون القط ذو الاسم Apricot، يجب علينا استخدام معلومه من جدولين. يمكننا ربط الجدولين باستخدام العامود `cat_id`، ويكون لدينا جدول جديد يحتوي على `name` و `color`.

### الربط

الربط بين عدة جداول باستخدام قيم في عواميدها.
...
**مثال:** نريد الربط بين جدولي `names` و `colors` لجمع كل قطه مع لونها. بما أن كلا الجدولين يحتويان على العامود `cat_id` وهو رقم مميز لكل قط، يمكننا استخدام الربط الداخلي باستخدام العامود `cat_id`.
...
لاحظ في النتيجة النهاية ظهور القيم 3 و 4 في عامود `cat_id`. إذا كانت أحد القيم تظهر في جدول دون الآخر، فيتم إضافتها للجدول النهائي مع القيمة `NULL` بدلاً من القيم المفقودة.
...
هذه المرة، الجدول النهائي يظهر جميع الألوان. ثلاثة من القيم في العامود `cat_id` بجدول `colors` لديها مطابق في عامود `cat_id` بجدول `names` وقيمه واحد لم تجد مطابق (white). اللون الذي لم يجد له اسم مطابق سيعوض بالقيمة `NULL`.
...
الطريقة الأخرى *Implicit* لكتابة هذا الأمر يكون شكلها مختلف وبدون `INNER JOIN`. لاحظ أن `FORM` يستخدم فيها الفواصل بين جدولين والشرط `WHERE` يحدد شروط الربط:
...
يطلق على هذه العملية *بالضرب الديكارتي Cartesian product*، كل سطر في الجدول الأول يربط بكل سطر في الجدول الثاني. لاحظ أن الكثير من الأسطر تحتوي على ألوان قطط لا تتطابق مع أسمائها. الشرط الإضافي في `WHERE` يطبق الربط ويفلتر القيم التي لا تتطابق في عامود `cat_id`:
