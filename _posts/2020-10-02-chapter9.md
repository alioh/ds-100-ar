...
- **Declarative تعرفيه**: قم بعملية حساب للأعمدة `x` و `y` من الجدول `A` عندما تكون القيمة في العامود `y` أكبر من 100.00.
...
لنقل أن لدينا عميل ذو ذوق عالي ويريد فقط تلك المتاجر التي تبيع منتجات بسعر أعلى من $700. لاحظ أن يجب علينا استخدام `HAVING` لإيجاد النتيجة من عامود تم تجميعه؛ لا يمكننا استخدام `WHERE` لفلترة نتائج عامود مُجَمع. لإيجاد قائمة بالمتاجر التي تبيع المنتجات المفضلة لعميلنا نقوم بالتالي:
