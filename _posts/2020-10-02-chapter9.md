...
*مخطط العلاقات Relation schema* تحتوي على أسماء الأعمدة، انواع البيانات وقيودها. مثلاً، مخطط جدول `purchases` يحدد ان هناك الأعمدة `name`، `product`، `retailer` و `date purchased`، ويوضح أيضاً ان كل عامود يحتوي على نص.
...
- **Imperative أمريه**: لكل قيمه في الجدول `A`، تحقق أن كانت القيمه للمتغير `y` أكثر من 100. إذا كانت كذلك، احفظ سمات الأعمدة `x` و `y` في جدول جديد. واظهر لنا الجدول الجديد.
...
- `SELECT` وتعني الأعمدة التي نريد اظهارها.
...
الأستعلام `SELECT *` يجلب لنا جميع الأعمدة. لعرض فقط عامود `retailer`، نقوم بإضافتها كالتالي:
...
في بانداز يمكننا استخدام الداله `pd.merge` لجمع جدولين معاً يحتويان على قيم متشابهه في أحد الأعمدة. مثلاً:
